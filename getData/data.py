...
conn = psycopg2.connect("host=localhost port=5432 dbname=foodmart user=postgres password=postgres target_session_attrs=read-write")
print('Connect')
        ...
        try:
            ...
            for item in vacancies_from_response:
                ...
                for vacancy in item:
                    ...
                    print({query_string})
                    cur.execute(
                        f"SELECT count(1) FROM vacancies_short WHERE vacancy_id={vacancy['id']} AND query_string='{query_string}'")
                    if cur.fetchone()[0] == 0:
                        print('нет данных')
                        ...
                        try:
                            ...
                            salary_gross = 0
                        try:
                            insider_interview_id = int(vacancy['insider_interview']['id'])
                        except TypeError:
                            insider_interview_id = 0
                        try:
                        ...
                        cur = conn.cursor()
                        try:
                            cur.execute(f'INSERT INTO vacancies_short VALUES {tuple_to_insert}')
                        except Exception as E:
                            print(f'!!NOT INSERT {E}')
        except Exception as E:
            print(f'!!!!! {E}')
